...
export const signup = async (input: CreateUserInput): Promise<AuthApiResponse> => {
    const {
        email,
        password,
        role,
...
export const login = async (input: CreateLoginInput): Promise<AuthApiResponse> => {
    const { email, password, userAgent = "chrome deafult", ip = "192.168.1.2 default" } = input;
    ...
    try {
        ...
        if (!isMatch)
            throw new AppError("INVALID_CREDENTIALS", 400, "Email or password is incorrect");
...
export const refreshSession = async (refreshToken: string) => {
    ...
    if (session.revokedAt) {
        throw new AppError("UNAUTHORIZED", 401, "Session is expired, try login again");
    }
